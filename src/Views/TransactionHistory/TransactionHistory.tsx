...
export default function TransactionHistory() {
  ...
  // API Queries
  const { data: TransactionHistorying, refetch } = useGetUsersQuery({
  ...
  return (
    <>
      ...
      <CustomTableView
        rows={(TransactionHistorying?.data?.data as unknown as Row[]) ?? []}
        ...
        renderTableFooter={() => (
          <ReactPaginate
            pageCount={(TransactionHistorying?.data?.count ?? 1) / USERS_PAGE_LIMIT}
            onPageChange={handlePageClick}
            activeClassName={STRINGS.ACTIVE}
            nextClassName={`${STRINGS.NEXT_BTN} ${
              Math.ceil((TransactionHistorying?.data?.count ?? 1) / USERS_PAGE_LIMIT) !==
              currentPage + 1
